...
var mainView = myApp.addView('.view-main'), isIOS;
	...
	var applicatFn = {
		...
		goPages: function (e) {
			...
			if (item.children().attr('data-child') == '1') {//关键任务打开弹窗
				...
				moveHrFn($$(this).children().children('p').html(),item.children().attr('data-children'))
	...
	$$('#appWrap').children().children().filter(function () {
		...
		if ($$(this).children().attr('data-child') == '1') {
			$$(this).click(function (event) {
				moveHrFn($$(this).children().children('p').html(),$$(this).children().attr('data-children'))
			});
...
// 移动Hrmodal
function moveHrFn(appName,data) {//移动Hr
	var templateModal = $$('#moveHr').html();
	var compiledModal = Template7.compile(templateModal);

	myApp.modal({
		title: '<h2 class="ifly-title clearfix"><span class="text">' + appName + '</span><a href="#" onclick="myApp.closeModal()"><i class="icon icon-close">×</i></a></h2>',
...
//    var url = 'https://tyapp.iflytek.com:443/TyTransService/tytranservice/transfer/transInterface' //cdn测试
